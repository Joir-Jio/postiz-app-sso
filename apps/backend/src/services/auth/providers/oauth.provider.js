...
class OauthProvider {
    constructor() {
        const { POSTIZ_OAUTH_AUTH_URL, POSTIZ_OAUTH_CLIENT_ID, POSTIZ_OAUTH_CLIENT_SECRET, POSTIZ_OAUTH_TOKEN_URL, POSTIZ_OAUTH_URL, POSTIZ_OAUTH_USERINFO_URL, FRONTEND_URL, } = process.env;
        if (!POSTIZ_OAUTH_USERINFO_URL)
            throw new Error('POSTIZ_OAUTH_USERINFO_URL environment variable is not set');
        if (!POSTIZ_OAUTH_URL)
        ...
        if (!POSTIZ_OAUTH_CLIENT_SECRET)
            throw new Error('POSTIZ_OAUTH_CLIENT_SECRET environment variable is not set');
